...
"
# maintainer generated deps packs
SRC_URI+="
	https://github.com/ruizhi-lab/gentoo-overlay/releases/download/${P}/${P}-deps.tar.xz
	https://github.com/ruizhi-lab/gentoo-overlay/releases/download/${P}/v2rayA-core-${PV}-vendor.tar.xz
"
...
src_install() {
	...
	# symlink geoip/geosite dat files for both:
	# - v2raya_core (xray-core): looks in /usr/share/xray/
	# - v2rayA service (XDG data): looks in /usr/share/v2raya/
	# actual files are in /usr/share/v2ray/ (provided by app-alternatives)
...
pkg_postinst() {
	...
	if has_version '<net-proxy/v2rayA-2.4.6' ; then
		...
		elog "The standalone net-proxy/v2ray and net-proxy/v2ray-bin packages"
		elog "are no longer required by v2rayA. You may remove them if they"
		elog "are not needed by other packages."
	fi
}
